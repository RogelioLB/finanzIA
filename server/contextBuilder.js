...
- Saldo neto del mes: ${(monthlyIncome > 0 ? (monthlyBalance / monthlyIncome) * 100 : 0).toFixed(1)}%
...
${
  creditCards && creditCards.length > 0
    ? creditCards
        .map((c) => {
          const utilization = ((c.current_balance / (c.credit_limit || 1)) * 100).toFixed(1);
...
OBJETIVOS ACTIVOS:
${
  objectives && objectives.filter((o) => !o.is_archived).length > 0
    ? objectives
        .filter((o) => !o.is_archived)
        .map((o) => {
          const progress = ((o.current_amount / (o.amount || 1)) * 100).toFixed(1);
...
${
  topCategories.length > 0
    ? topCategories
        .map(
          ([cat, total], i) =>
            `${i + 1}. ${cat}: ${formatCurrency(total)} (${(monthlyExpenses > 0 ? (total / monthlyExpenses) * 100 : 0).toFixed(1)}%)`
        )
        .join("\n")
 ...
 */
function getUpcomingPayments(transactions, creditCards, objectives) {
  const payments = [];
